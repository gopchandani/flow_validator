...
class FlowTable:
    def __init__(self, sw, table_id, flow_table_raw):
        ...
        if self.sw.network_graph.controller == "grpc":
            for flow_raw in flow_table_raw.flow_rules:
                f = Flow(sw, self, flow_raw)
                self.flows.append(f)
        else:
            for flow_raw in flow_table_raw:
                f = Flow(sw, self, flow_raw)
                self.flows.append(f)
